...
// ------------------------------------------------------------------
/**
 * Permanently empties the **complete** folder specified by
 * `COMPLETE_DRIVE_FOLDER_ID` by moving every file it contains to trash.
 * The workflow expects your Python script to download attachments from the
 * incoming folder, process them, and then place (or evoke a sync of) the
 * results into the Drive folder that corresponds to `Contracts/Complete`.
 * Once the files arrive there, this helper removes them from Drive so they
 * don't linger anywhere other than the original email thread.
 *
 * WARNING: this does not verify that processing succeeded. It simply trashes
 * whatever is present in the complete folder at the time it runs.
 */
